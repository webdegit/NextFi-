...
contract GlobalFiUpgradeableNew is
    Initializable,
    PausableUpgradeable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function setPools(
        uint8 _poolId,
        uint256 _rewardToDistributeInDecimals,
        uint8 _minUserCounter
    ) external onlyOwner {
        PoolStruct storage poolsAccount = _mappingPools[_poolId];

        poolsAccount.poolId = _poolId;
        poolsAccount.rewardToDistribute =
            _rewardToDistributeInDecimals *
            1 ether;
